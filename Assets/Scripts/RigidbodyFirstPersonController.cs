...
public class RigidbodyFirstPersonController : MonoBehaviour
{
	...
	private bool m_Jump, m_PreviouslyGrounded, m_Jumping, m_IsGrounded, m_Climbing;
...
//		Debug.DrawRay (transform.position + (transform.forward*0.5f)+(transform.up*2), -Vector3.up, Color.white, 5);
		if(Physics.Raycast(transform.position + (transform.forward*0.5f)+(transform.up*2), -Vector3.up, out hitB))
		{
			print (hitB.distance + hitB.collider.gameObject.name);
			if(hitB.distance <  1.0f && Physics.Raycast(hitB.point - new Vector3(0, 0.1f, 0) - (transform.forward*0.5f), transform.forward, out hitA))
			{
				Debug.DrawRay (hitB.point - new Vector3(0, 0.1f, 0) - (transform.forward*0.5f), transform.forward, Color.red, 5);
				...
				{
					m_RigidBody.velocity += new Vector3(0, 4.0f, 0);
				}
			}
		}
	}
...
//		m_RigidBody.useGravity = false;
		transform.position = edge.transform.position - (transform.forward*0.5f) - (transform.up *1.6f);
		GameObject test = (GameObject)Instantiate (testPos, transform.position, transform.rotation);
...
//				transform.position = Vector3.MoveTowards (transform.position, edge.transform.position + transform.forward * 0.5f + transform.up * 1.6f, 5f * Time.deltaTime);
		...
		m_Climbing = false;
		mouseLook.YSensitivity = 2.0f;
		transform.rotation = Quaternion.Euler(0.0f, cam.transform.eulerAngles.y, 0.0f);
		cam.transform.rotation = Quaternion.Euler(cam.transform.eulerAngles.x, transform.eulerAngles.y, cam.transform.eulerAngles.z);
